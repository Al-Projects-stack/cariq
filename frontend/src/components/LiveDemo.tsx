...
const FAKE_Q = "Is R185k fair for a 2019 Polo 1.0 TSI in Cape Town?";
const FAKE_A = `**Short answer:** Yep — that's a **GOOD DEAL** if it's a clean service history.
      ...
      <div className="mt-10 rounded-2xl border border-gray-800 bg-gradient-to-br from-gray-900 to-gray-950 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800 bg-gray-950/50">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-3 font-mono text-xs text-gray-500">Live demo — entirely fake, entirely fun</span>
          </div>
        ...
        <div className="grid lg:grid-cols-5 gap-0">
          ...
          <div className="lg:col-span-2 border-b lg:border-b-0 lg:border-r border-gray-800 p-5 bg-gray-900/30">
            <p className="font-mono text-xs tracking-widest uppercase text-gray-500">You ask</p>
            <div className="mt-3 rounded-xl border border-gray-800 bg-gray-950 p-4">
              <p className="text-sm text-gray-200 leading-relaxed">“{FAKE_Q}”</p>
              <p className="mt-3 text-xs text-gray-500">Hint: this one’s staged — sign in to ask the real RAG anything. It’s less shy.</p>
            </div>
            ...
            {!user && (
              <Link to="/signup" className="mt-5 inline-flex w-full justify-center rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white hover:bg-orange-400 transition-colors shadow-lg shadow-orange-500/20">
                Sign in to ask for real
              </Link>
            )}
            {user && <p className="mt-5 text-xs text-green-400 font-medium">You’re in — scroll down and fire a real query.</p>}
          </div>

          {/* right: answer */}
          <div className="lg:col-span-3 p-5">
            <p className="font-mono text-xs tracking-widest uppercase text-gray-500">CarIQ answers</p>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <p className="mt-3 text-xs text-gray-600">Staged example — real answers are grounded in 20-model SA knowledge base and cite sources.</p>
          </div>
        </div>
      </div>
    </div>
